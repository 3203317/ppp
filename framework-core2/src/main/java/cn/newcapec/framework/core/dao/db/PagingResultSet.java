  ...
  implements Serializable, LogEnabled
{
  ...
  {
    if (this.pageSize > 0) {
      this.pageCount = (this.currRow % this.pageSize != 0 ? this.currRow / this.pageSize + 1 : this.currRow / this.pageSize);
    }
    else
      this.pageCount = 1;
  }
  ...
  private void updateList() throws Exception
  {
    try {
      ...
      for (int i = 0; (i < this.pageSize) && (isLast()); i++)
        this.list.add(BeanUtils.resultSetToDO(this.resultSet, this.modelClass, true));
    }
    finally
    {
    }
    ...
    throws SQLException
  {
    ...
    else { int startRow = this.pageSize * (this.pageIndex - 1);

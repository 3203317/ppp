...
public class ClassUtils {
		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					System.err.println("加载数据:扫描"+packageName+"内class...");
					// 获取包的物理路径
	...
	public static List<Class<?>> getAllClassByInterface(Class<?> c) {
		List<Class<?>> returnClassList = ObjectUtils.getArrayList();
		// 如果不是一个接口，则不做处理
		if (c.isInterface()) {
			...
			try {
				...
				for (Class<?> class1 : allClass) {
					if (c.isAssignableFrom(class1)) { // 判断是不是一个接口
						if (!c.equals(class1)) { // 本身不加进去
							returnClassList.add(class1);
						}
					}
				}
	...
	public static void main(String[] args) {
		Set<Class<?>> cObjects = getClasses("com.qingshu.base.pojo");
		for (Iterator<Class<?>> iterator = cObjects.iterator(); iterator.hasNext();) {
